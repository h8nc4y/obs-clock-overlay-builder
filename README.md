...
- Date display can be tuned by independent axes: year on/off, month/day zero-padding, `/` / `-` / Japanese separators, weekday brackets, and AM/PM position for 12-hour clocks.
- Fine text tuning per part: label / date / time groups with optional per-part weight and letter-spacing overrides, a date-weekday gap slider, and an AM/PM size slider (default 0.55× of the time digits).
...
- 日付は「年を表示する」「月・日をゼロ埋め」「/・-・日本語(Y年M月D日)の区切り」を別々に選べます。曜日は（）で囲め、12時間表示では AM/PM を時刻の前(AM 09:51)にも置けます。以前の共有URLはそのまま同じ見た目で動きます。
- 「文字調整：ラベル / 日付 / 時刻」で部位ごとの太さ・文字間隔(既定はおまかせ連動)、日付と曜日の間隔、AM/PM の大きさ(既定は時刻の約半分)を細かく調整できます。
...
- 生成された `/clock/?c=...` URL とシェア画像には、ラベルに入力した文字がそのまま含まれます。URL や画像を公開するときは、未公開の配信予定など見られて困る情報をラベルに書かないでください。
