...
> 更新: 2026-07-12 / 作成者: Claude (Fable 5, 司令塔) + Codex / 想定読者: Codex（実装主担当）
>
...
**日々の実装は Codex 自走、意匠は Claude、本番デプロイだけオーナーGO** — この3行が全て。詳細:

| 観点 | 運用 |
|---|---|
| **開発の主導** | Codex 単独（タスク分解・実装・テスト・docs・コミット・ブランチ・PR・版上げ準備）。逐一の承認待ちは不要。 |
| **レビュー** | 原則 Codex のセルフレビュー。`node --test`＋§5 のチェックリストを必ず通す。 |
| **フロントの「デザイン」** | 意匠判断（配色・タイポ・余白/サイズ感・レイアウトの美観・新テンプレの見た目・共有画像の構図・色コントラスト調整）は Codex が独断で決めず、**(1) 作業を止め (2) オーナーに貼れる形で依頼 (3) 回答を待って再開**。依頼テンプレ＝「対象 / 現状 / 制約（依存ゼロ・CSP・`clock.css` 凍結・再現性契約）/ 狙い」。返ってきた意匠（color hex・タイポ・layout・signature）を実装 → テスト → 共有Canvas鏡像化 → golden再生成 → 出荷。意匠変更のコミット/PRには由来の痕跡（例: trailer `Design: Claude Design (owner approved)`）を残す。 |
| **Codex 単独でよいもの** | ロジック/状態/配線、サニタイズ・コーデック・正規化、テスト・golden fixture、共有Canvasの**ロジック鏡像化**（ライブ `clock.css` の値へ合わせ直す整合作業は意匠ではない）、ドキュメント、リリース機構、バグ修正。 |
