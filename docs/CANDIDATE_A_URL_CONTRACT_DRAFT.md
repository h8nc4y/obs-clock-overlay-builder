...
## Internal Dispatch Overlay Runtime URL Boundary

internal dispatch overlay runtime connection 後も、generated URL は config-only を維持する。

次PR候補の `demo=1` internal dispatch path でも、URLへ入れないもの:
