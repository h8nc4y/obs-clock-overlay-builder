...
This is planning and implementation planning evidence for future direction. The matching normalization, fixture playback scope, overlay runtime scope, single synthetic event scope, event source shape, queue / transport, overlay queue connection, transport scope, local intake queue connection, local intake overlay runtime connection, first transport decision, internal dispatch overlay runtime connection, manual input dispatch scope, and fixture dispatch scope policies are not adoption evidence, and they do not claim advanced Unicode normalization, overlay runtime event source implementation, transport implementation, YouTube Live integration, YouTube API access, or broad adoption.
