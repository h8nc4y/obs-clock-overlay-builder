# ClaudeCode Fable5 prompt - 011_obs-clock-overlay-builder
...
UIあり。ClaudeDesign でOBS配信者向けに設定密度、プレビュー、URLコピー、透明背景、スマホ確認を再設計する。`clock.css` の描画契約は凍結寄り。
...
対象 repo は `011_obs-clock-overlay-builder` です。まず `docs/CLAUDECODE_FABLE5_HANDOFF.md` の repo handoff を読み、そこにある要件資料、Web調査/判断資材、設計資料、タスク一覧、進捗、残タスク、gate、reading order を作業の出発点にしてください。既存資料は現状把握の根拠であり、再要件定義後の正本ではありません。
